...
export default function MatchingChapter() {
  ...
  const { isComplete } = useProgress('matching', chapterNum);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <MascotGuide activity="matching" phrase="Pick a level!" />
      <View style={[styles.header, { backgroundColor: Colors.tileMatching }]}>
        ...
        {levels.map((level) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/matching/level?chapter=${chapterNum}&level=${level.id}`)}
              activeOpacity={0.8}
            >
              <AnimalCartoon name={level.mascot} size={tileSize * 0.5} />
...
const styles = StyleSheet.create({
  ...
  tileDone: { backgroundColor: '#FFFDE7', borderWidth: 2, borderColor: Colors.tileMatching },
  tileTitle: { fontSize: FontSizes.xs, color: Colors.textMuted, textAlign: 'center' },
  doneMark: { fontSize: 14, color: Colors.tileMatching, fontWeight: '800' },
});
